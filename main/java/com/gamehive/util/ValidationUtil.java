...
public class ValidationUtil {
	...
	/**
	 * Validates if the provided password meets complexity requirements: - Contains
	 * at least one uppercase letter (A-Z) - Contains at least one lowercase letter
	 * (a-z) - Contains at least one digit (0-9) - Contains at least one symbol
	 * (@$!%*?&).
	 * 
	...
	public static boolean isValidPassword(String password) {
		return password.matches("^(?=.*[a-z])(?=.*[A-Z])(?=.*\\d)(?=.*[@$!%*?&])[A-Za-z\\d@$!%*?&]*$"); // No length
																										// validation
	}
	...
	public static boolean isGenderMatches(String gender) {
		// Convert the gender to lowercase and compare with "male" and "female" ignoring
		// case
		return gender != null && (gender.equalsIgnoreCase("male") || gender.equalsIgnoreCase("female"));
	}
	
	/**
	 * Checks if the provided user is above the age of 10 or not.
	 * 
	 * @param date of birth of the user to validate
	 * @return True if the user is above 10, false otherwise.
	 * */
